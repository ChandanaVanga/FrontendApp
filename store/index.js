...
const actions = {
  ...
  async GetAllToDOs({ commit, state }) {
    try {
      ...
      const res = await axios.get('http://localhost:5000/api/todolist/', {
        headers: {
  ...
  async DeleteToDOs(state, data) {
    try {
      ...
      console.log(id);
      const res = await axios.delete('http://localhost:5000/api/todolist/' + id, {
        headers: {
          Authorization: "Bearer " + this.state.token,
        },
  ...
  async CreateToDo(state, data) {
    try {
      ...
      const res = await axios({ method: 'post', url: 'http://localhost:5000/api/todolist', headers: headers, data: data.data })
